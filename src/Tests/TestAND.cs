...
public class TestAND : Test {
	...
	public static void TestANDReg8Reg8(CPURegister source, CPURegister destination) {
		var tests = new (byte, byte, Dictionary<CPUFlag, bool>)[]
		{
			...
			(0b01, 0b10, new() { {CPUFlag.Zero, true}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
			(0x42, 0x69, new() { {CPUFlag.Zero, false}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
			(0xE9, 0x07, new() { {CPUFlag.Zero, false}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
			(0xFF, 0xFF, new() { {CPUFlag.Zero, false}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
		};
		...
		foreach (var test in tests) {
			...
			RunOperation(gameboy,
				...
				new DataTypeReg8(destination), test.Item1, test.Item2, (ushort)(test.Item1 & test.Item2), test.Item3, new FlagPermissionHandler("Z010"));
		}
	}

	public static void TestANDU8Reg8(CPURegister destination) {
		var tests = new (byte, byte, Dictionary<CPUFlag, bool>)[]
		{
			(0xFF, 0x00, new() { {CPUFlag.Zero, true}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
			(0b01, 0b10, new() { {CPUFlag.Zero, true}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
		...
		foreach (var test in tests) {
			...
			RunOperation(gameboy,
				...
				test.Item1,
				test.Item2,
				(ushort)(test.Item1 & test.Item2),
				test.Item3,
				new FlagPermissionHandler("Z010")
			);
		}
	}

	public static void TestANDReg16AddressReg8(CPURegister source, CPURegister destination, ushort sourceAddress) {
		var tests = new (byte, byte, Dictionary<CPUFlag, bool>)[]
		{
			(0xFF, 0x00, new() { {CPUFlag.Zero, true}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
			(0b01, 0b10, new() { {CPUFlag.Zero, true}, {CPUFlag.NSubtract, false}, {CPUFlag.HalfCarry, true}, {CPUFlag.Carry, false} }),
	...
	public override void Run() {
		...
		TestANDReg16AddressReg8(CPURegister.HL, CPURegister.A, Memory.WorkRAMBank0.Address);
